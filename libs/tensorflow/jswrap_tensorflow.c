...
const unsigned char g_sine_model_data[] = {
...
JsVarFloat jswrap_tensorflow_test(JsVarFloat x) {
  size_t s = tf_get_size(2048, g_sine_model_data_len);
  char *d = (char *)alloca(s);
  tf_create(d, 2048, g_sine_model_data);
  tf_get_input(d,0)->data.f[0] = x;
  tf_invoke(d);
  float out = tf_get_output(d,0)->data.f[0];
  tf_destroy(d);
  return out;
}

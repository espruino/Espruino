...
void lcdSetPixels_ArrayBuffer(JsGraphics *gfx, JsVar *buf, short x, short y, short pixelCount, unsigned int col) {
...
void lcdSetPixel_ArrayBuffer(JsGraphics *gfx, short x, short y, unsigned int col) {
  JsVar *buf = jsvObjectGetChild(gfx->graphicsVar, "buffer", 0);
  if (buf && jsvIsArrayBuffer(buf)) {
    lcdSetPixels_ArrayBuffer(gfx, buf, x, y, 1, col);
  }
  jsvUnLock(buf);
}
...
void  lcdFillRect_ArrayBuffer(struct JsGraphics *gfx, short x1, short y1, short x2, short y2) {
  ...
  if (buf && jsvIsArrayBuffer(buf)) {
    short y;
    for (y=y1;y<=y2;y++)
      lcdSetPixels_ArrayBuffer(gfx, buf, x1, y, (short)(1+x2-x1), gfx->data.fgColor);
  }
  jsvUnLock(buf);

...
int  net_ESP8266_BOARD_recv(JsNetwork *net, int sckt, void *buf, size_t len, uint32_t *host, unsigned short *port);
int  net_ESP8266_BOARD_send(JsNetwork *net, int sckt, const void *buf, size_t len, uint32_t host, unsigned short port);
void net_ESP8266_BOARD_idle(JsNetwork *net);
bool net_ESP8266_BOARD_checkError(JsNetwork *net);
int  net_ESP8266_BOARD_createSocket(JsNetwork *net, uint32_t ipAddress, unsigned short port, SocketType socketType, JsVar *options);

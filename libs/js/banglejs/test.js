// Bangle.js test script that is run by BANGLEJS2_TEST build (minimal test build)
Terminal.println("Bangle.js test image");
setTimeout(function() {
  Terminal.println("Starting...");
//  E.showMessage("Erasing Storage");
//  require("Storage").eraseAll();
  ...
  var pass = [];
  const TESTS = "TS,GPS,HRM,Baro,Mag,Acc,Btn,Chg,Vibrate,BLE".split(",");
  const abs = Math.abs;
  ...
  Bangle.on('swipe', e => {
    ...
    setTimeout(function() {
      ...
      g.clear(1).setFont("6x8:3").setFontAlign(0,0).drawString("TEST PASS\nNO FW\nDONT SHIP",88,88);

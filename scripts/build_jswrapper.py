...
# Add main types
for className in classes:
  symbolTables[className] = { "type" : "object", "name" : className, "functions" : [] }
...
#    exit(1);
  ...
  symbolTables[libName] = { "type" : "library", "name" : libName, "functions" : [] }
...
for className in objectChecks.keys():
  if not className=="global": # we did 'global' above
    if (className+".prototype") in classes: # FIXME FIXME

...
void console_readToQueue(){
  char rxChar;STATUS status;int charCnt;
  status = OK; charCnt = 0;
  while(status == OK && charCnt++ < uartRxCharMax){
    status = uart_rx_one_char((uint8_t *)&rxChar);
	if (status == OK) {
      /*if((int)rxChar == 17){consoleRxIdx = EspruinoIOs[EspruinoMaster].rx;printf("toMaster\n");}
	  else if((int)rxChar == 18){consoleRxIdx = EspruinoIOs[EspruinoSlave].rx;printf("toSlave\n");}
      else // preparation for switching between different Esruino tasks (or others ?) */ 
	  queue_writeChar(consoleRxIdx,rxChar);
	}
  }
  return;
}
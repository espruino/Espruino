...
static ble_uuid_t serviceFilter;
static ble_uuid_t charFilter;
...
void gattc_evt_search_res(esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param) {
  ...
  // no need to check since esp_ble_gattc_search_service accepts a filter
...
void gattc_connect(ble_gap_addr_t peer_addr, JsVar *options) {
  ...
  bleaddr_TO_espbtaddr(peer_addr, remote_bda, &remote_bda_type);
	esp_err_t ret;
	ret = esp_ble_gattc_open((esp_gatt_if_t)gattc_apps[GATTC_PROFILE].gattc_if, remote_bda, remote_bda_type, true);
	jsble_check_error((uint32_t)ret);
}
uint32_t gattc_disconnect(uint16_t conn_handle) {
  NOT_USED(conn_handle);
	esp_err_t ret;
	ret = esp_ble_gattc_close((esp_gatt_if_t)gattc_apps[GATTC_PROFILE].gattc_if,gattc_apps[GATTC_PROFILE].conn_id);
	jsble_check_error((uint32_t)ret);
	return (uint32_t)ret;
}
void gattc_searchService(ble_uuid_t uuid){
  ...
  bleuuid_TO_espbtuuid(uuid, &espServiceFilter);
	esp_err_t ret = esp_ble_gattc_search_service((esp_gatt_if_t)gattc_apps[GATTC_PROFILE].gattc_if, gattc_apps[GATTC_PROFILE].conn_id, &espServiceFilter);
	jsble_check_error((uint32_t)ret);
	// followed by ESP_GATTC_SEARCH_RES_EVT
	// and ESP_GATTC_SEARCH_CMPL_EVT
}
void gattc_getCharacteristic(ble_uuid_t char_uuid){
	uint16_t count = 0;
	charFilter = char_uuid;
	esp_ble_gattc_get_attr_count( 
	    (esp_gatt_if_t)gattc_apps[GATTC_PROFILE].gattc_if, gattc_apps[GATTC_PROFILE].conn_id,ESP_GATT_DB_CHARACTERISTIC,
	...
	if(count > 0) {
	  ...
	  bleuuid_TO_espbtuuid(char_uuid,&espCharFilter);
		esp_ble_gattc_get_char_by_uuid(
		    (esp_gatt_if_t)gattc_apps[GATTC_PROFILE].gattc_if, gattc_apps[GATTC_PROFILE].conn_id,
			gattc_apps[GATTC_PROFILE].service_start_handle,gattc_apps[GATTC_PROFILE].service_end_handle,
			espCharFilter,char_elem_result,&count);
		if(count > 0){
...
// check for connected
	esp_ble_gattc_read_char((esp_gatt_if_t)gattc_apps[GATTC_PROFILE].gattc_if,gattc_apps[GATTC_PROFILE].conn_id,
...
void gattc_readDesc(uint16_t charHandle) {
	esp_ble_gattc_read_char_descr ((esp_gatt_if_t)gattc_apps[GATTC_PROFILE].gattc_if,gattc_apps[GATTC_PROFILE].conn_id,
	charHandle,ESP_GATT_AUTH_REQ_NONE);
}

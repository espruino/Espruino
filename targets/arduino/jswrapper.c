...
#if( 'q\0\0\0' & 'q' )
  #error( "architecture is big-endian. need to test and make sure this works" )
#endif
// beware big endian!
...
JsVar *jswHandleFunctionCall(JsVar *parent, JsVar *parentName, const char *name) {
  if (parent) {
    ...
    if (CMP4(name[0],'c','l','o','n') && CMP2(name[4],'e','\0')) {
      ...
      jspParseEmptyFunction();
      JsVar *_r = jswrap_object_toString(parent);
      return _r;
    }
    ...
    if (jsvIsFunction(parent)) {
      if (CMP4(name[0],'r','e','p','l') && CMP4(name[4],'a','c','e','W') && CMP4(name[8],'i','t','h','\0')) {
    ...
    if (jsvIsArray(parent)) {
      ...
      } else if (CMP4(name[0],'s','p','l','i') && CMP3(name[4],'c','e','\0')) {
    ...
    if (CMP4(parent->varData.str[0],'M','a','t','h') && parent->varData.str[4]=='\0') {
      if (name[0]=='a') {
        ...
        JsVar *_r = jsvNewFromFloat(pow(jsvGetFloatAndUnLock(x), jsvGetFloatAndUnLock(y)));
        return _r;
        ...
        if (CMP3(name[1],'i','n','\0')) {
          ...
          JsVar *x = jspParseSingleFunction();
          JsVar *_r = jsvNewFromFloat(pow(jsvGetFloat(x),0.5));
          jsvUnLock(x);
          return _r;
        }
      } else if (name[0]=='r') {
    ...
    if (CMP4(parent->varData.str[0],'c','o','n','s') && CMP4(parent->varData.str[4],'o','l','e','\0')) {
      if (CMP4(name[0],'l','o','g','\0')) {
        ...
        JsVar *text = jspParseSingleFunction();
    ...
    if (CMP4(name[0],'A','r','r','a') && CMP4(name[4],'y','B','u','f') && CMP4(name[8],'f','e','r','\0')) {
      ...
      return _r;
    } else if (CMP4(name[0],'a','n','a','l') && CMP2(name[4],'o','g')) {
    ...
    } else if (CMP3(name[0],'b','i','t')) {
      if (CMP4(name[3],'S','e','t','\0')) {
        // bitSet  (src/jswrap_interactive.c)
        JsVar *value, *bit;
        jspParseFunction(0, &value, &bit, 0, 0);
        jswrap_interface_bitSet(value, jsvGetIntegerAndUnLock(bit));
        jsvUnLock(value);
        return 0;
      } else if (CMP4(name[3],'R','e','a','d') && name[7]=='\0') {
        // bitRead  (src/jswrap_interactive.c)
        JsVar *value, *bit;
        jspParseFunction(0, &value, &bit, 0, 0);
        JsVar *_r = jsvNewFromInteger(jswrap_interface_bitRead(jsvGetIntegerAndUnLock(value), jsvGetIntegerAndUnLock(bit)));
        return _r;
      } else if (CMP4(name[3],'C','l','e','a') && CMP2(name[7],'r','\0')) {
        // bitClear  (src/jswrap_interactive.c)
        JsVar *value, *bit;
        jspParseFunction(0, &value, &bit, 0, 0);
        jswrap_interface_bitClear(value, jsvGetIntegerAndUnLock(bit));
        jsvUnLock(value);
        return 0;
      } else if (CMP4(name[3],'W','r','i','t') && CMP2(name[7],'e','\0')) {
        // bitWrite  (src/jswrap_interactive.c)
        JsVar *value, *bit, *data;
        jspParseFunction(0, &value, &bit, &data, 0);
        jswrap_interface_bitWrite(value, jsvGetIntegerAndUnLock(bit), jsvGetIntegerAndUnLock(data));
        jsvUnLock(value);
        return 0;
      } else if (name[3]==0) {
        // bit  (src/jswrap_interactive.c)
        JsVar *bit = jspParseSingleFunction();
        JsVar *_r = jsvNewFromInteger(jswrap_interface_bit(jsvGetIntegerAndUnLock(bit)));
        return _r;
      }
    } else if (name[0]=='e') {
    ...
    } else if (CMP4(name[0],'g','e','t','T') && CMP4(name[4],'i','m','e','\0')) {
    ...
    } else if (CMP4(name[0],'h','i','g','h') && CMP4(name[4],'B','y','t','e') && name[8]=='\0') {
      // highByte  (src/jswrap_interactive.c)
      JsVar *value = jspParseSingleFunction();
      JsVar *_r = jsvNewFromInteger(jswrap_interface_highByte(jsvGetIntegerAndUnLock(value)));
      return _r;
    } else if (CMP4(name[0],'m','e','m','o') && CMP3(name[4],'r','y','\0')) {
      // memory  (src/jswrap_interactive.c)
      jspParseEmptyFunction();
      JsVar *_r = jswrap_interface_memory();
      return _r;
    } else if (CMP2(name[0],'l','o')) {
      if (CMP3(name[2],'a','d','\0')) {
        // load  (src/jswrap_interactive.c)
        jspParseEmptyFunction();
        jsiSetTodo(TODO_FLASH_LOAD);
        return 0;
      } else if (CMP4(name[2],'w','B','y','t') && CMP2(name[6],'e','\0')) {
        // lowByte  (src/jswrap_interactive.c)
        JsVar *value = jspParseSingleFunction();
        JsVar *_r = jsvNewFromInteger(jswrap_interface_lowByte(jsvGetIntegerAndUnLock(value)));
        return _r;
      }
      ...
      if (CMP4(name[1],'a','r','s','e')) {
        if (CMP4(name[5],'I','n','t','\0')) {
          ...
          JsVar *_r = jsvNewFromInteger(jswrap_parseInt(string, jsvGetIntegerAndUnLock(radix)));
          jsvUnLock(string);
          return _r;
      ...
      if (CMP4(name[1],'a','v','e','\0')) {
        ...
        if (CMP4(name[3],'I','n','t','e') && CMP4(name[7],'r','v','a','l') && name[11]=='\0') {
          ...
          jsvUnLock(pin);
          return 0;
        } else if (CMP4(name[3],'B','u','s','y') && CMP4(name[7],'I','n','d','i') && CMP4(name[11],'c','a','t','o') && CMP2(name[15],'r','\0')) {
      ...
      if (CMP4(name[2],'s','e','t','\0')) {
        // reset  (src/jswrap_interactive.c)
        jspParseEmptyFunction();
        jsiSetTodo(TODO_RESET);
        return 0;
      } else if (CMP4(name[2],'g','i','s','t') && CMP3(name[6],'e','r','\0')) {
        // register  (src/jswrap_interactive.c)
        JsVar *code = jspParseSingleFunction();
        jswrap_interface_register(code);
        jsvUnLock(code);
        return 0;
      }
    } else if (name[0]=='U') {

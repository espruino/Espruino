...
// Softdevice doesn't support USB power events :(
...
static bool last_usb_conn_status;
...
static bool usb_connection_handle(bool last_usb_conn_status)
{
    if (last_usb_conn_status != m_usb_connected)
    {
        ...
        if (m_usb_connected) {
          jsiConsolePrintf("USB start\n");
          app_usbd_start();
        } else {
          jsiConsolePrintf("USB stop\n");
          app_usbd_stop();
        }
    }

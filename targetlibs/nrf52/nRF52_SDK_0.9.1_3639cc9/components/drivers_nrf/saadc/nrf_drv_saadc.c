...
typedef struct
{
    ...
    nrf_saadc_value_t *           buffer;                        ///< Sample buffer.
    uint16_t                      buffer_size;                   ///< Size of the sample buffer.
    uint16_t                      buffer_pos;                    ///< Current sample buffer position.
    nrf_saadc_value_t *           p_secondary_buffer;            ///< Secondary sample buffer.
...
void SAADC_IRQHandler(void)
{
    if (nrf_saadc_event_check(NRF_SAADC_EVENT_END))
    {
        ...
        if (m_cb.active_channels == 1)
        {
            ...
            evt.data.done.p_buffer = m_cb.buffer;
        ...
        else
        {
            ...
            m_cb.buffer_pos++;
            if (m_cb.buffer_pos == m_cb.buffer_size)
            {
                nrf_drv_saadc_evt_t evt;
                evt.type = NRF_DRV_SAADC_EVT_DONE;
                evt.data.done.p_buffer = m_cb.buffer;
                ...
                {
                    (void)nrf_drv_saadc_buffer_convert(m_cb.p_secondary_buffer,m_cb.secondary_buffer_size);
                }
                m_cb.event_handler(&evt);
            }
            else
            {
                ...
                nrf_saadc_buffer_init(m_cb.buffer + m_cb.buffer_pos, 1);
...
void nrf_drv_saadc_limit_set(uint8_t channel, int16_t limit_low, int16_t limit_high)
{
    ...
    uint32_t int_mask = int_mask = nrf_saadc_limit_int_get(channel, NRF_SAADC_LIMIT_LOW);

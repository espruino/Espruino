...
uint32_t nrf_sdk_mem_init(void)
{
    MM_LOG("[MM]: >> nrf_sdk_mem_init.\r\n");
...
uint32_t nrf_sdk_mem_alloc(uint8_t ** pp_buffer, uint32_t * p_size)
{
    ...
    MM_LOG("[MM]: >> nrf_sdk_mem_alloc, size 0x%04lX.\r\n", requested_size);
    ...
    MM_LOG("[MM]: << nrf_sdk_mem_alloc %p, result 0x%08lX.\r\n", (*pp_buffer), err_code);

    return err_code;
}


uint32_t nrf_sdk_mem_free(uint8_t * p_buffer)
{
    VERIFY_MODULE_INITIALIZED();
    NULL_PARAM_CHECK(p_buffer);

    MM_LOG("[MM]: >> nrf_sdk_mem_free %p.\r\n", p_buffer);
    ...
    MM_LOG("[MM]: << nrf_sdk_mem_free, result 0x%08lX.\r\n", err_code);
    return err_code;
}

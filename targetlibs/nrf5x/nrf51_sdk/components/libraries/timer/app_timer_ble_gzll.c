...
#define SWI1_IRQ_PRI            APP_IRQ_PRIORITY_LOW                            /**< Priority of the SWI0 interrupt (used for updating the timer list). */
 ...
 *          but the module can easily be modified to have one queue per process when using an RTOS. 
 *          The purpose of the queues is to be able to have a completely lockless timer
 *          implementation.
 */
typedef struct
{
...
/**@brief Function for initializing and starting the RTC1 counter.
 *
...
{
    if (ticks_now >= ticks_old)
    {
        return (ticks_now - ticks_old);
    }
    else
    {
        return (MAX_RTC_CNT + 1) - (ticks_old - ticks_now);
    }
}

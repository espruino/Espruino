 ...
 */
#if defined ( __CC_ARM )
__asm void Security_SVC_Handler(void)
{
SEC_SVC_HANDLER   EQU 0x0003DC00    ; Location of nRF Security library, SVC handler is located at this address + 0x2C.
SVCall_IRQ_OFFSET EQU 0x2C          ; Address offset of SVC handler
    
    LDR   R2, =SEC_SVC_HANDLER      ; Get address of the forward address variable
    ADDS  R2, #SVCall_IRQ_OFFSET    ; SVC handler is at pos 11, which is offset (4 * 11) bytes
    LDR   R2, [R2]                  ; Load address of Application SVC Handler

...
#define ANTFS_NETWORK_KEY                  {0x00, 0x00, 0x00, 0x00,\
                                            0x00, 0x00, 0x00, 0x00}      /**< Set to the ANT-FS network key!!! */
...
#ifdef LEDDRIVER_ACTIVE
                leddriver_led_turn_on(LED_ID_1);
#endif // LEDDRIVER_ACTIVE
                break;
...
static void link_layer_transit(void)
{
    ...
    {
#ifdef LEDDRIVER_ACTIVE
        leddriver_led_turn_off(LED_ID_1);
#endif // LEDDRIVER_ACTIVE
...
void antfs_channel_setup(void)
{
    ...
    err_code = sd_ant_channel_id_set(ANTFS_CHANNEL,
                                     device_number,
                                     ANTFS_DEVICE_TYPE,
                                     ANTFS_TRANS_TYPE);
    APP_ERROR_CHECK(err_code);
    ...
    err_code = sd_ant_channel_open(ANTFS_CHANNEL);
    APP_ERROR_CHECK(err_code);

    err_code = sd_ant_channel_radio_tx_power_set(ANTFS_CHANNEL,
                                                 ANTFS_TRANSMIT_POWER,
                                                 ANTFS_CUSTOM_TRANSMIT_POWER);
    APP_ERROR_CHECK(err_code);

...
#define NRF_NFCT_ERRORSTATUS_ALL    (NFCT_ERRORSTATUS_EOFERROR_Msk          | \
...
hal_nfc_retval hal_nfc_setup(hal_nfc_callback callback, void *cbContext)
{
    ...
    uint8_t  nfc_internal[T2T_INTERNAL_BYTES_NR] = {0};                         // Buffer for internal bytes used in NFC message header
...
/* End: Bugfix for FTPAN-17 */
    ...
    nfc_internal[0] = (uint8_t) (LSB(nfc_tag_header0 >> T2T_INTERNAL_BYTE_SN0_SHIFT));         //SN0
    nfc_internal[1] = (uint8_t) (LSB(nfc_tag_header0 >> T2T_INTERNAL_BYTE_SN1_SHIFT));         //SN1
    nfc_internal[2] = (uint8_t) (LSB(nfc_tag_header0 >> T2T_INTERNAL_BYTE_SN2_SHIFT));         //SN2
    nfc_internal[3] = (uint8_t) ((CASCADE_TAG_BYTE) ^ nfc_internal[0] ^ 
                                  nfc_internal[1]   ^ nfc_internal[2]);                        //BCC0 = CASCADE_TAG_BYTE ^ SN0 ^ SN1 ^ SN2
    nfc_internal[4] = (uint8_t) (LSB(nfc_tag_header1 >> T2T_INTERNAL_BYTE_SN3_SHIFT));         //SN3
    nfc_internal[5] = (uint8_t) (LSB(nfc_tag_header1 >> T2T_INTERNAL_BYTE_SN4_SHIFT));         //SN4
    nfc_internal[6] = (uint8_t) (LSB(nfc_tag_header1 >> T2T_INTERNAL_BYTE_SN5_SHIFT));         //SN5
    nfc_internal[7] = (uint8_t) (LSB(nfc_tag_header1 >> T2T_INTERNAL_BYTE_SN6_SHIFT));         //SN6

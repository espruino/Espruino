...
JsVar *jspeUnary() {
    if (execInfo.lex->tk=='!') {
        ...
        if (JSP_SHOULD_EXECUTE)
          return jsvNegateAndUnLock(jspePostfix());
        else
          return jspePostfix();
    } else
        return jspePostfix();
...
__attribute((noinline)) JsVar *__jspeExpression(bool negate, JsVar *a) {
  if (negate) {
...
JsVar *jspeExpression() {
    ...
    return __jspeExpression(negate, jspeTerm());
}

  ...
  while (*fmt) {
    if (*fmt == '%') {
      ...
      case 'L': {
        ...
        itoa(va_arg(argp, JsVarInt), buf, rad);
        user_callback(buf, user_data);
      }
      break;
      case 'f':
        ftoa(va_arg(argp, JsVarFloat), buf);
        user_callback(buf, user_data);
        break;
      case 's':
        user_callback(va_arg(argp, char *), user_data);
        break;
      case 'c':
        buf[0] = (char)va_arg(argp, int/*char*/);
        buf[1] = 0;
        user_callback(buf, user_data);
        break;
      case 'v': {
      ...
      break;
      case 't':
        user_callback(jsvGetTypeOf(va_arg(argp, JsVar*)), user_data);
        break;
      case 'p':
        jshGetPinString(buf, (Pin)va_arg(argp, int/*Pin*/));
        user_callback(buf, user_data);
        break;
      default:
        assert(0);
